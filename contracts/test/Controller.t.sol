...
contract ControllerTest is Test {
    ...
    function setUp() public {
        ...
        OptionsFactory optionsFactory = OptionsFactory(controller.optionsFactory());
        ...
        (qTokenXAddress, cTokenIdX) =
            optionsFactory.createOption(address(WBNB), oracle, uint88(block.timestamp + 604800), true, 500 ether);
    ...
    function testCannotExerciseMoreThanBalance() public {
        ...
        vm.mockCall(
            priceRegistry,
            abi.encodeWithSelector(bytes4(keccak256(bytes("hasSettlementPrice(address,uint88,address)")))),
            abi.encode(true)
        );
        ...
        PriceWithDecimals memory settlementPrice = PriceWithDecimals(20000000000, 8);

        vm.mockCall(
            priceRegistry,
            abi.encodeWithSelector(bytes4(keccak256(bytes("getSettlementPriceWithDecimals(address,uint88,address)")))),
            abi.encode(settlementPrice)
        );

        PriceWithDecimals memory returnedPrice = IPriceRegistry(priceRegistry).getSettlementPriceWithDecimals(
            address(BUSD), uint88(futureTimestamp), address(controller)
        );

        assertEq(returnedPrice.price, settlementPrice.price);
        assertEq(uint256(returnedPrice.decimals), uint256(settlementPrice.decimals));
    }
    ...
    function testCannotPassInvalidActionType() public {
        ...
        args[0] = ActionArgs(ActionType.Call, address(32), address(64), address(96), uint256(128), uint256(160), "0xc0");
    ...
    function testCannotNeutralizeMoreThanBalance() public {
        ...
        bytes32 userCTokenBalanceSlot = keccak256(abi.encode(cTokenIdX, keccak256(abi.encode(user, balanceOfSlot))));
        ...
        vm.store(address(collateralToken), userCTokenBalanceSlot, bytes32(userCTokenBalance));
